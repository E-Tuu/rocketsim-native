...
def materials():
    ...
    return SingleStageRocketMaterials(POLYSTYRENE, CARDBOARD, CARDBOARD)
...
def test_aggregate_equations(geometry, materials):
    ...
    components = (result.nose, result.body, result.fins)
    ...
    assert result.structure_cg_x_geo_m != sum(c.cg_x_geo_m for c in components) / 3
...
def test_material_redistribution(geometry, materials):
    ...
    changed = SingleStageRocketMaterials(CARDBOARD, CARDBOARD, POLYSTYRENE)
    result = evaluate(geometry, changed)
    ...
    expected = sum(m*x for m,x in zip(masses, (geometry.nose_volume_centroid_x_geo_m,
        geometry.body_volume_centroid_x_geo_m, geometry.fin_set_volume_centroid_x_geo_m))) / sum(masses)
...
def test_uniform_density_scale(geometry):
    ...
    a = evaluate(geometry, SingleStageRocketMaterials(low, low, low))
    b = evaluate(geometry, SingleStageRocketMaterials(high, high, high))
...
def test_result_contracts(geometry, materials):
    ...
    assert [f.name for f in fields(StructuralMassProperties)] == [
        'nose', 'body', 'fins', 'structure_mass_kg', 'structure_cg_x_geo_m']
    result = evaluate(geometry, materials)
    for obj in (result, result.nose, result.body, result.fins):
...
def test_success_invariants(geometry, materials):
    ...
    result = evaluate(geometry, materials)
    for mass in (result.nose.mass_kg, result.body.mass_kg, result.fins.mass_kg, result.structure_mass_kg):
...
def test_aggregate_overflow(geometry):
    ...
    with pytest.raises(MassValidationError) as caught:
        evaluate(geometry, SingleStageRocketMaterials(material, material, material))
